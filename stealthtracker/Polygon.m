...
- (id)initWithFrame:(CGRect)frame polygon:(UIImage *)image value:(int)integer label:(NSString *)string
{
    ...
    if (self)
    {
        ...
        label.frame = CGRectMake(self.frame.origin.x + self.frame.size.width/2 - label.frame.size.width/2 - 10, self.frame.origin.y + self.frame.size.height - 10, label.frame.size.width + 20, label.frame.size.height);
